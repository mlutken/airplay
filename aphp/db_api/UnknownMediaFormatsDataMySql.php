...
// 'unknown_media_format_name', 'record_store_id', 'buy_at_url' 
...
*/
class UnknownMediaFormatsDataMySql extends SimpleTableDataMySql
{
    // ------------------------
    // --- Constructor init --- 
    // ------------------------
    public function __construct( $dbPDO = null )
    {
        ...
        , array( 'unknown_media_format_name', 'record_store_id', 'buy_at_url' ) 
        , $dbPDO );
    }
...
// //     public function erase ($media_format_id)
// //     {
// //         $aArgs = array($media_format_id);
// //         $stmt = $this->m_dbPDO->prepare( 'DELETE FROM media_format WHERE media_format_id = ?' );
// //         $stmt->execute( $aArgs );
// //     }
...
// //     }
    ...
    public function setData ($unknown_media_format_name, $record_store_id, $buy_at_url )
    {
        $stmt = $this->m_dbPDO->prepare("
        REPLACE INTO unknown_media_format  
        SET unknown_media_format_name = ?, record_store_id = ?, buy_at_url = ?" );
        $stmt->execute( array($unknown_media_format_name, $record_store_id, $buy_at_url) );
