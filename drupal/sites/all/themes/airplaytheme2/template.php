 ...
 * See 'function airplaytheme2_col_width' and layout.css for setting the column widths.
 *
 * @param $left, $content, $right, $right_2
 *   The main column regions
 */
function airplaytheme2_column_count_class($left, $content, $right, $right_2) {
 ...
 *   The main column regions
 */
function airplaytheme2_col_width($left, $content, $right, $right_2) {
  if ($content && $left && $right && $right_2) {
 ...
 */
if (is_null(theme_get_setting('airplaytheme2_color_global'))) {  // <-- change this line
  global $theme_key;
...
$color_global = theme_get_setting('airplaytheme2_color_global');
$color_highlight = theme_get_setting('airplaytheme2_color_highlight');
// Add the style sheets.
drupal_add_css(drupal_get_path('theme', 'airplaytheme2') .'/css/'. $color_global .'.css', 'theme');
drupal_add_css(drupal_get_path('theme', 'airplaytheme2') .'/css/'. $color_highlight .'.css', 'theme');
 ...
 */
function airplaytheme2_preprocess_page(&$vars) {
  global $theme;
  
  // AP TODO: Changed ML
  $airplay_site_name = "Airplay Music";  // $vars['site_name']
...
//      $vars['site_name'] = '<h1 class="brand"><a href="'. $vars['front_page'] .'" title="'. $vars['site_name'] .'">'. $vars['site_name'] .'</a></h1>'; 
      $vars['site_name'] = '<h1 class="brand"><a href="'. $vars['front_page'] .'" title="'. $vars['site_name'] .'">'. $airplay_site_name .'</a></h1>'; 
    }
  }
  else {
    if ($vars['logo']) {
...
//      $vars['site_name'] = '<div class="brand"><a href="'. $vars['front_page'] .'" title="'. $vars['site_name'] .'">'. $vars['site_name'] .'</a></div>'; 
      $vars['site_name'] = '<div class="brand"><a href="'. $vars['front_page'] .'" title="'. 'BBB' .'">'. $airplay_site_name .'</a></div>'; 
    }
  }
...
//      $vars['title'] = '<h1 class="title category clearfix">'. $vars['feed_icons'] .'<span>'. $vars['title'] .'</span></h1>';
      $vars['title'] = '<h1 class="title category clearfix">'. $vars['feed_icons'] .'<span>'. $airplay_site_name .'</span></h1>';
    }
    else {
  ...
  $body_class = array($vars['body_class']);
    ...
    if (isset($vars['node']) && $vars['node']->type) {
      $body_class[] = 'node-type-'. airplaytheme2_id_safe($vars['node']->type);
    }
    if (!$vars['is_front']) {
    ...
    $body_class[] = airplaytheme2_id_safe('page-'. $path);
    $body_class[] = airplaytheme2_id_safe('section-'. $section);
    if (arg(0) == 'node') {
      if (arg(1) == 'add') {
        if ($section == 'node') {
 ...
 * @see http://drupal.org/project/zen
 */
function airplaytheme2_preprocess_node(&$vars) {
  global $user;
 ...
 *  @see http://drupal.org/project/zen
 */
function airplaytheme2_preprocess_comment(&$vars) {
  global $user;
 ...
 * @see http://drupal.org/project/zen
 */
function airplaytheme2_preprocess_block(&$vars) {
  $block = $vars['block'];
 ...
 * @see http://drupal.org/project/zen
 */
function airplaytheme2_id_safe($string) {
