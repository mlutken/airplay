...
for TBL in $(echo "show tables" | \
   mysql -h${HST} -u${USR} -p${PWD} ${DB_DRUPAL} | grep -v -e ${SKIP})
do
  echo "drop table ${TBL}" | mysql -h${HST} -u${USR} -p${PWD} ${DB_DRUPAL}
done
 
# now I can load the script
mysql --default-character-set=utf8 -h${HST} -u${USR} -p${PWD} ${DB_DRUPAL} < $1
