...
function airplaymusic_form($variables) {
...
adds form-required
*/
function airplaymusic_form_element($variables) {
  ...
  if(! theme_get_setting('airplaymusic_classes_form_wrapper_formitem')){
  ...
  if (!empty($element['#type'])) {
    if(!theme_get_setting('airplaymusic_classes_form_wrapper_formtype')){
      $attributes['class'][] = 'form-type-' . strtr($element['#type'], '_', '-');
    }
  }
  if (!empty($element['#name'])) {
    if(!theme_get_setting('airplaymusic_classes_form_wrapper_formname')){
  ...
  if($attributes['class']){
    $remove_class_form = explode(", ", theme_get_setting('airplaymusic_classes_form_freeform'));
  ...
  if (!empty($element['#description'])) {
    ...
    */
    if(!theme_get_setting('airplaymusic_classes_form_description')){
...
*/
function airplaymusic_form_element_label($variables) {
  $element = $variables['element'];
  ...
  if(!theme_get_setting('airplaymusic_form_required')){
...
//       $attributes['class'] = 'form-field-required';
  ...
  if ($element['#title_display'] == 'after') {
    if(!theme_get_setting('airplaymusic_classes_form_label')){
  ...
  if (!empty($element['#id'])){
    ...
    if(
      ...
      theme_get_setting('airplaymusic_html5') AND
      $element['#type'] != "item" &&
...
*/
function airplaymusic_textfield($variables) {
  $element = $variables['element'];
  $element['#size'] = '30';
  ...
  $element['#attributes']['type'] = 'text';
	...
	//placeholder
  if (!empty($element['#title']) AND theme_get_setting('airplaymusic_classes_form_placeholder_label') ) {
  ...
  if(!theme_get_setting('airplaymusic_classes_form_input')){
...
function airplaymusic_link_field($vars) {
  ...
  if (empty($element['#field']['multiple'])) {
	...
	//placeholder
	if( theme_get_setting('airplaymusic_classes_form_placeholder_link') ){
    $element['#attributes']['placeholder'] = theme_get_setting('airplaymusic_classes_form_placeholder_link');
	}
	elseif (!empty($element['#title']) AND theme_get_setting('airplaymusic_classes_form_placeholder_label') ) {
   $element['#attributes']['placeholder'] = $element['#title'];
  }
...
*/
function airplaymusic_emailfield($variables) {
  $element = $variables['element'];
  ...
  _form_set_class($element, array('form-text', 'form-email'));
	...
	//placeholder
	if( theme_get_setting('airplaymusic_classes_form_placeholder_email') ){
    $element['#attributes']['placeholder'] = theme_get_setting('airplaymusic_classes_form_placeholder_email');
	}
	elseif (!empty($element['#title']) AND theme_get_setting('airplaymusic_classes_form_placeholder_label') ) {
   $element['#attributes']['placeholder'] = $element['#title'];
  }
...
function airplaymusic_urlfield($variables) {
  $element = $variables['element'];
...
function airplaymusic_textarea($variables) {
  $element = $variables['element'];
...
function airplaymusic_checkbox($variables) {
  $element = $variables['element'];
  $t = get_t();
...
function airplaymusic_radio($variables) {
  $element = $variables['element'];
...
function airplaymusic_file($variables) {
  $element = $variables['element'];
//  $element['#size'] = '30';
...
*/
function airplaymusic_password($variables) {
  $element = $variables['element'];
  $element['#size'] = '30';
...
//  element_set_attributes($element, array('id', 'name',  'maxlength'));
  ...
  if (!empty($element['#description']) AND theme_get_setting('airplaymusic_classes_form_placeholder_description') ) {
...
/* removed form-select */
function airplaymusic_select($variables) {
  $element = $variables['element'];
  element_set_attributes($element, array('id', 'name', 'size'));
...
remove form-text class
*/
function airplaymusic_text_format_wrapper($variables) {
  $element = $variables['element'];
...
function airplaymusic_button($variables) {
  $element = $variables['element'];
...
remove form-wrapper
*/
function airplaymusic_fieldset($variables) {
  $element = $variables['element'];
...
function airplaymusic_container($variables) {
  $element = $variables['element'];
  ...
  if (isset($element['#array_parents'])) {
    ...
    // Add the 'form-wrapper' class.
		if(!theme_get_setting('airplaymusic_classes_form_container_wrapper')){
    	$element['#attributes']['class'][] = 'form-wrapper';
		}

		//remove the field-type-...  yup this is but ugly
		if(theme_get_setting('airplaymusic_classes_form_container_type')){
			$element['#attributes']['class']['0'] = "";
		}

		//remove the field-name-field...  yup this is but ugly
		if(theme_get_setting('airplaymusic_classes_form_container_name')){
			$element['#attributes']['class']['1'] = "";
		}

		//remove the field-widget-....
		if(theme_get_setting('airplaymusic_classes_form_container_widget')){
			$element['#attributes']['class']['2'] = "";
		}

		//remove the id
		if(theme_get_setting('airplaymusic_classes_form_container_id')){
...
function airplaymusic_field_multiple_value_form($variables) {
  $element = $variables['element'];
	$output = '';
...
*/
function airplaymusic_form_alter(&$form, &$form_state, $form_id) {
/*
	print "<pre>";
	print_r($form_id);
	print_r($form);
...
*/
  ...
  if ($form_id == 'user_login_block') {
	...
	if($form_id == 'user_register_form'){
		//placeholder
		if(theme_get_setting('airplaymusic_classes_form_placeholder_email')){
			$mail_placeholder = theme_get_setting('airplaymusic_classes_form_placeholder_email');
		}else{
